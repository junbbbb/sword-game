...
// ★★14차(브롤스타즈 화풍). 0.20 -> 0.30. 계약은 그대로인데 **화면에서 묽어졌다** —
//   씬 휘도가 여섯 배가 되면서 같은 가산량의 상대 비중이 그만큼 줄었다
//   (실측: 웜 풀 화면 진폭 19.7% -> 2.9%. scripts/d2_pulse.py 로 잰 같은 자다).
//   폭을 반쯤 되찾되 **블룸 계약**(임계 1.02, 알파 **최대**로 잰다)을 지킨다:
//       웜 풀 1.06 x 알파 0.720 x 맥동 1.30 = 0.992 < 1.02  ✓
//       후광   1.06 x 알파 0.720 x 맥동 1.30 = 0.992 < 1.02  ✓
//   그래서 s40 의 세기를 1.15 -> 1.06 으로 같이 내렸다(둘은 한 짝이다).
//   ★밝은 카툰에서 옛 ±20% 수준의 화면 진폭(19.7%)을 되찾으려 들면 안 된다 —
//     그 값은 어둠이 배경이던 판의 값이고, 여기서 그만큼 뛰면 바닥이 껌뻑인다.
const POOL_PULSE = 0.30;         // 바닥 웜 풀 맥동 ±30%
const HALO_PULSE = 0.30;         // 불꽃 후광. 불꽃 바로 옆이라 웜 풀과 같이 뛴다
