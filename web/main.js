...
const ATK_COMMIT = { Attack: 0.30, Heavy: 0.53, Wide: 0.47 };
...
const ATK_STEP_T = [0, 0.50, 1.10];
const ATK_STEP_COMMIT = [0.30, 0.20, 0.24];
...
function tryAttack() {
  ...
  // ── 크로스페이드 0.06 -> 0.10 (2026-08-12 14-베기수정) ──
  ...
  if (current && current !== a) current.crossFadeTo(a, 0.10, false);
  current = a;
...
function tryHeavy() {
  ...
  // 크로스페이드 0.08 -> 0.12 (근거는 tryAttack 의 같은 자리 주석)
  if (current && current !== a) current.crossFadeTo(a, 0.12, false);
  current = a;
...
function tryWide() {
  ...
  // 크로스페이드 0.08 -> 0.16 (근거는 tryAttack 의 같은 자리 주석)
  if (current && current !== a) current.crossFadeTo(a, 0.16, false);
  current = a;
...
function tick() {
  ...
  if (heavy) gainTarget = 1.85;
    ...
    if (current && current === actions.Attack) {
      ...
      gainTarget = u > 0.65 ? 1.80 : (u > 0.33 ? 1.55 : 1.35);
    }
  }
