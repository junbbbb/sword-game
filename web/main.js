...
const TRAIL_MAX = 22;              // 보관 샘플 수(원본)
const RIBS = 140;                  // 실제로 그리는 마디 수. 원본을 곡선 보간해 늘린다.
                                   // 22개를 그대로 이으면 빠른 호에서 각이 눈에 띈다.
  ...
  fragmentShader: `
    ...
    void main(){
      ...
      if (kind < 0.5) {
        ...
        // ★v92. 바깥 끝을 0.06 폭으로 흐려 없애던 자리다. 그 부드러운 알파가
        //   "속이 비치는 유리 부채"의 절반이었다. 이제 **형태의 끝은 선으로 끊는다**
        //   (아래 먹 밴드가 그 자리를 차지한다). 0.016 은 계단이 안 보일 만큼만
        //   남긴 안티에일리어싱 폭이다 - 넓히면 다시 반투명 띠가 된다.
        // ★v94. 안티에일리어싱 폭을 0.016 -> 0.006 으로 더 좁혔다. 이 부드러운 한 겹이
        //   화면에서 **경계 1픽셀을 배경과 반반 섞어** 놓아서, 먹 밴드를 아무리 두껍게
        //   그려도 심사관이 재는 '경계 링'은 중간 명도로 나왔다(먹선 비율 11~14%).
        //   계단은 씬 타겟 MSAA 4배가 받아 준다.
        float ea = 1.0 - smoothstep(outer - 0.004, outer + 0.002, v);
        if (!(ea > 0.02)) discard;
        ...
        if (uUseTex > 0.5) {
          ...
          // ── 만화 채색 (v92. 오너: "만화 같은 그래픽 + 외곽선 렌더링") ──
          // ★알파는 **형태의 유무**만 가른다. 은은하게 비치는 반투명을 쓰는 순간
          //   그림이 아니라 '유리 부채'가 된다(오너가 지적한 그 그림이다).
          //   텍스처 알파를 문턱으로 끊어서 구멍은 구멍대로 남기고 몸은 통짜로 칠한다.
          float aa = min(1.0, vA * ea * step(0.45, tx.a) * 1.55);
          if (!(aa > 0.03)) discard;      // ★NaN 도 걸러내는 형태(!(a > x))
          // ── 색 계단 ──
          // ★텍스처를 실측하고 정했다(2048x256, 알파는 사실상 이진 - 불투명 94%).
          //   가로 방향 띠(연하늘-시안-진파랑-남색-흰 포말)는 **이미 평칠로 구워져
          //   있다.** 그러니 여기서 밝기를 다시 뭉개면 애써 그린 띠 구조가 통째로
          //   한 색으로 합쳐진다(실측 밝기 중앙값 0.324 / 0.377 / 0.409 가 한 칸에
          //   들어온다). 매끄럽게 미끄러지던 건 **길이 방향 감쇠 하나**뿐이라
          //   그것만 네 단으로 끊는다. 이제 꼬리가 그라데이션이 아니라 단으로 진다.
          float aq = floor(age * 4.0) / 4.0;                   // 0 / 0.25 / 0.5 / 0.75
          vec3 ct = tx.rgb * (1.0 - 0.30 * aq);                // 꼬리로 갈수록 한 단씩 가라앉는다
          ct = pow(ct, vec3(1.18));                            // 감청 쪽으로 한 단 눌러 채도를 살린다
          // ── 외곽선 렌더링 ──
          // ★외곽선은 얹는 장식이 아니라 **형태를 정의하는 선**이다. 이 리본이
          //   "반투명 파란 부채"로 보이던 진짜 이유도 색이 아니라 경계였다 - 바깥 끝을
          //   알파로 흐려 없앴으니 배경과 만나는 선이 아예 없었다.
          //   리본 스트립의 **가장자리로부터의 거리**로 굵은 먹 밴드를 두른다.
          //     · 머리(방금 지나간 자리)는 굵고 꼬리로 갈수록 얇다(붓이 빠지는 결)
          //     · 텍스처가 찢겨 뚫린 자리도 경계다(옆 텍셀이 비면 거기가 실루엣이다)
          //   두께 전환은 1~2px 폭으로만 준다. 넓히면 그라데이션이 되어 도로아미타불이다.
          float tn = clamp(v / max(outer, 1e-3), 0.0, 1.0);    // 0=안쪽 가장자리 1=바깥
          float dE = min(tn, 1.0 - tn) * 2.0;                  // 0=가장자리 1=한가운데
          // ★v94. 0.30->0.10 이던 밴드를 0.44->0.20 으로 넓혔다. 심사 격차 2:
          //   "경계 1px 링 어두운 화소 비율 - 귀멸 40~63%, 게임 파랑 베기 11~14%."
          //   먹선은 얹는 장식이 아니라 **형태를 정의하는 선**이라 폭이 곧 판정이다.
          float ow = mix(0.44, 0.20, age);                     // 먹 밴드 굵기(머리->꼬리)
          float na = min(min(texture2D(uTex, tuv + vec2(0.0, 0.030)).a,
                             texture2D(uTex, tuv - vec2(0.0, 0.030)).a),
                         min(texture2D(uTex, tuv + vec2(0.005, 0.0)).a,
                             texture2D(uTex, tuv - vec2(0.005, 0.0)).a));
          // ★전환 폭도 0.05 -> 0.02. 넓히면 먹과 색 사이에 중간 명도가 생기고,
          //   그 중간 명도가 곧 '경계가 흐린 CG 띠'다.
          float ik = max(1.0 - smoothstep(ow, ow + 0.02, dE), step(na, 0.45));
          gl_FragColor = vec4(mix(ct, C_DK2 * 0.45, ik), aa);
          return;
        }
        vec3 c;
        // ── 잉크 외곽선(양쪽) + 밝은 갓선. 원작의 결정적 특징 ──
        // ★v92. 고정 폭(0.075 / 0.045)이던 것을 **머리는 굵고 꼬리는 얇게** 바꿨다.
        //   텍스처를 안 입는 칼들(얼음·독·흙·어둑)도 같은 외곽선 문법을 따라야 한다.
        float ib = mix(0.15, 0.07, age);
        if (t > 1.0 - ib || t < ib * 0.6) c = C_DK2;
        else if (t > 0.91 - ib) c = fire ? C_WHT : C_LT3;
        else {
      ...
      if (kind < 1.5) {
        ...
        for (int k = 0; k < 5; k++) {
          ...
          // ★v92. 제일 잔 결(u*63 · v*44)을 u*24 · v*17 로 낮췄다. 계단으로 끊고 나니
          //   그 주파수가 그대로 **점묘 얼룩**이 됐다(부드럽던 시절엔 안 보이던 결이다).
          float lump = noise(vec2(u * 11.0 + fk * 17.0 + sd, v * 7.0)) * 0.62
                     + noise(vec2(u * 27.0 + fk * 7.0 + sd, v * 19.0)) * 0.27
                     + noise(vec2(u * 24.0 + fk * 3.0 + sd, v * 17.0)) * 0.11;
          float edge = 0.62 + 0.62 * lump;
          a = max(a, smoothstep(edge + 0.06, edge - 0.06, r));
          rmin = min(rmin, r / max(edge, 1e-3));
        }
        // ★v92. 덩어리 경계(smoothstep)와 구멍을 **계단으로** 끊는다. 예전에는
        //   포말 가장자리가 0.12 폭으로 부드럽게 사라져서, 꼬리에서 다섯 가닥이
        //   전부 창백한 실선이 되고 그 사이로 배경이 비쳤다(실측: z_010 꼬리 확대).
        //   덩어리는 있거나 없거나 둘 중 하나여야 흰 포말이 '칠한 덩어리'로 읽힌다.
        if (!(a > 0.03)) discard;
        // ★v92. 구멍 주파수를 44x30 -> 12x9 로 낮췄다. 문턱으로 끊는 순간 44개짜리
        //   잔 구멍이 꼬리에서 **말뚝 울타리**(가로로 늘어선 막대 20여 개)가 됐다
        //   (실측: t1 z_010). 구멍은 '그린 구멍'이라야 하므로 크고 드물어야 한다.
        float hole = noise(vec2(u * 12.0 + sd * 9.0, v * 9.0));
        a = step(0.5, a) * step(0.30, hole);
        if (!(a > 0.03)) discard;
        vec3 c;
        if (fire) {
          c = rmin > 0.86 ? C_DK2
          ...
          // ★v94. 0.74 -> 0.60 으로 한 번 더 넓히고 색도 한 단 더 눌렀다(C_DK2*0.5).
          //   가닥 배치가 바뀌면서 **리본 실루엣의 바깥 끝이 이제 포말**이다
          //   (STRAND_CFG 의 off ±0.96·0.86 두 가닥). 즉 심사관이 재는 '경계 1픽셀 링'의
          //   대부분이 여기서 나온다. 몸통 먹선만 두껍게 해 봐야 안쪽에 묻힌다.
          if (rmin > 0.60) c = C_DK2 * 0.5;
          else {
            c = C_WHT;
            if (hole < 0.46) c = C_LT2;
            if (rmin > 0.44) c = C_LT1;
          }
        }
      ...
      // ★v92. 여기도 끝을 흐리지 않는다(0.02 -> 0.004 폭). 바깥 한 겹은 먹.
      float ea2 = 1.0 - smoothstep(lim - 0.004, lim, dd2);
      if (!(ea2 > 0.02)) discard;
      // ★v94. 먹 띠를 30% -> 42% 로 넓혔다(가는 심도 경계 링에 잡힌다).
      vec3 c2 = (dd2 < lim * 0.34) ? C_WHT : (dd2 < lim * 0.58 ? C_LT3 : C_DK2 * 0.5);
...
const sprayMat = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    void main(){
      ...
      // ★v94. 윤곽선 띠를 넓혔다(0.435/0.355 -> 0.400/0.338). 물보라 조각 수십 개가
      //   화면에 흩어져 있어서 '경계 1픽셀 링'의 상당 부분을 이 조각들이 만든다.
      bool edge = (r1 > 0.400) || (r2 < 0.338);          // 윤곽선 띠
      vec3 c;
      if (vInk > 0.5) {
      ...
      if (uRise > 0.5) {
        ...
        // ★v94. 윤곽을 uPal[1] 에서 uPal[0]*0.7 로 한 단 더 눌렀다(먹).
        c = edge ? uPal[0] * 0.7 : uPal[6];
...
const TRAIL_LADDER   = [1.00, 1.00, 0.92, 0.78, 0.34, 0.18];  // 세기(= 알파 계단의 입력)
const TRAIL_LADDER_W = [1.00, 1.00, 0.94, 0.80, 0.30, 0.16];  // 폭 배수
// ── 가닥 배치 (v94. 화면공간) ──
// 예전 SPAN 은 **칼날 축 위의 구간**이었다(그래서 리본이 칼날을 폭으로 썼다).
// 이제 각 가닥은 "칼날 위 어디에 중심선을 두고(at), 화면 수직으로 얼마나 밀려서(off),
// 얼마나 굵은가(half)"로 적는다. off·half 단위는 **미터**다.
//   이 시점(fov 24 · dist 24)에서 화면 세로 반높이가 약 5.1m 이므로
//   바깥 끝 |off|+half = 1.30m -> 일반 베기(gain 1.55) 때 띠 전체가 화면 높이의 약 40%,
//   일격기(gain 2.4) 때 약 61% 를 세로로 쓴다. 이게 '화면 존재감'의 출처다.
// ★off 는 **바깥 방향 한쪽으로만** 잰다(플레이어에서 멀어지는 쪽이 +).
//   0 근처면 칼날 위, 클수록 캐릭터에서 멀다. 캐릭터를 안 덮는 것이 이 부호의 목적이다.
//   바깥 끝 max(off+half) = 1.36m. 중심선이 몸에서 약 1.1m 이므로
//   일반 베기(gain 1.55)에서 바깥 끝이 몸에서 3.2m = **판정 리치와 같은 자리**에 선다.
const STRAND_CFG = [
  { kind: 0, at: 0.62, off: 0.22, half: 0.36, headOnly: 0 },   // 몸통 안쪽
  { kind: 0, at: 0.72, off: 0.70, half: 0.42, headOnly: 0 },   // 몸통 바깥쪽
  { kind: 2, at: 0.67, off: 0.46, half: 0.11, headOnly: 0 },   // 가는 심(흰 선)
  { kind: 1, at: 0.80, off: 1.16, half: 0.20, headOnly: 0 },   // 포말 마루(제일 바깥)
  // ★캐릭터 쪽 머리 장식. 리치를 못 늘리는 대신 **손 언저리**에 물이 뭉치게 한다.
  //   갓 지나간 구간(age < 0.38)에만 살아서 꼬리에는 안 따라간다.
  { kind: 1, at: 0.30, off: 0.02, half: 0.30, headOnly: 1 },
];
...
function updateTrail(force) {
  ...
  for (let j = n - 1; j >= 0; j--) {
    ...
    if (aa >= TRAIL_LADDER.length) break;
    if (ee.t * TRAIL_LADDER[aa > 0 ? aa : 0] <= 0.13) break;
    aliveN++;
  }
  ...
  for (let s = 0; s < STRANDS; s++) {
    ...
    for (let i = 0; i < RIBS; i++) {
      ...
      const li = af > 0 ? (af >= TRAIL_LADDER.length ? -1 : af) : 0;
      ...
      let q = inten > 0.46 ? 1.0 : (inten > 0.26 ? 0.92 : (inten > 0.13 ? 0.78 : 0));
  ...
  window.__trailDbg = { n, wrote: w, q: trailQFrame, hold: 0, clipped,
