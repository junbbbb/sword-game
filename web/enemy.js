...
// 경직 반동 시간. 60ms. 이보다 길면 "맞았다"가 아니라 "고무"가 된다.
const SQUASH_T = 0.06;
  ...
  const pipMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        float cell = floor(x);
        float fx = fract(x);
        if (fx < 0.09 || fx > 0.91) discard;          // 칸 사이 여백
        // 먹 테두리 안에 속을 채운다. 찬 칸 = 종이색 / 빈 칸 = 옅은 먹
        float inner = (fx > 0.21 && fx < 0.79 && vU.y > 0.22 && vU.y < 0.78) ? 1.0 : 0.0;
        vec3 ink = vec3(0.050, 0.042, 0.052);
        vec3 fill = cell < vHp - 0.5 ? vec3(0.94, 0.90, 0.80) : vec3(0.20, 0.17, 0.19);
        gl_FragColor = vec4(mix(ink, fill, inner), vA);
      }`,
  });
    ...
    for (let s = 0; s < steps; s++) {
      ...
      for (let i = live.length - 1; i >= 0; i--) {
        ...
        if (killed) {
          ...
          e.kb.set(kx, 0, kz).multiplyScalar(4.2);
          e.kbT = 0.13;
          // 경직 반동. 60ms 동안 납작해졌다가 돌아온다(y 0.85 / x 1.12).
  ...
  function update(dt, ctx) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (e.sqT > 0) {
        ...
        v.grp.scale.set(base * (1 + 0.12 * k), base * (1 - 0.15 * k), base * (1 + 0.12 * k));
        // 젖힘: 밀려나는 방향(kb)으로 상체가 넘어간다. 최대 0.22rad.
        const kl = Math.hypot(e.kb.x, e.kb.z);
        if (kl > 1e-4) {
          v.grp.rotation.set(0, e.yaw, 0);
          v.grp.rotateOnWorldAxis(_lean.set(-e.kb.z / kl, 0, e.kb.x / kl), 0.22 * k);
        }
      } else if (wu > 0) {
