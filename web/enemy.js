...
//   ② 머리 위 바의 길이 계약(핍 1 = PIP_W = 0.22m)이 그대로 산다. maxHp 가 안 변하니
//      **바의 물리적 크기가 화소 하나 안 바뀐다.** 체력을 2배로 올렸다면 리더 바가
//      0.66m -> 1.32m 로 두 배가 돼서 PIP_W 까지 같이 손봐야 했다(회귀 두 배).
...
export function createEnemySystem(opts) {
  ...
  function updatePlates() {
    ...
    for (let i = 0; i < live.length; i++) {
      ...
      //   → 2핍 놈의 만피 바와 3핍 놈이 1핍 깎이고 남은 채움은 **같은 길이**다.
      // ★18차(DMG_SCALE 0.5): 이 계약은 **비례식이라 배율에 안 흔들린다.** 바뀐 건
      //   화면 길이당 타수의 환산비 하나뿐이다(0.22m = 1대 -> 2대). 3핍 리더 바는
      //   여전히 0.66m 이고, 거기서 "남은 채움 길이 ∝ 남은 타수"는 그대로 성립한다.
      //   그래서 PIP_W 는 손대지 않았다 - 건드리면 바 크기가 실제로 변한다.
