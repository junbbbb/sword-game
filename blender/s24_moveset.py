...
# ══════════════════════════════════════════════════════════════════════════
# ★★26차 HEAVY_V26 — X **주력기 승격 재작: 오버헤드 대각 클리브** (2026-08-25,
#   오너 "z 스킬은 없애고 x 스킬 즉 위에서 아래로 베는거 다시해줘")
#   Z 가 빠지면 X 가 유일 근접 주력기다. 검도 골격(치켜들기-한 박자-낙하-잔심-회수)은
#   유지하되 진폭·체중을 주력기감으로 키운다. `HEAVY_V26=0` 이면 24·25차 X 그대로.
#
# ── 25차 커밋본 실측(repro26_2_s24.log)이 정한 물리 한계 ──
#   ①낙하 즉발: f8->f9 Δbel 30 으로 첫 장부터 최고속 근처(가속 서사가 짧다).
#   ②바닥이 깊이를 캡한다: f12 bel -35 에서 이미 z -0.014 (손 1.06m 일 때 월드
#     칼끝 고도 -40도가 관통 0 의 하한). 설계 초안의 bel -48 마무리는 이 손높이에서
#     z -0.13 = 13cm 관통이라 **바닥 관통 금지 계약이 이긴다** — 오버슛 한 장만
#     -0.02~-0.03(스침, 게임 접지 흡수 한도 내)까지 내리고 정착은 z +0.02 로 되돌린다.
#     대신 낙차는 챔버를 +78->+84 로 올리고 가속을 한 장 늦춰(hot f10~12) 번다.
#   ③잔심 4장 -> 3장(죽은 장 감축), 회수는 cr 감소가 bel 을 끌어올리는 [R10] 곡선.
#
# ── 26차 X 규칙 ──
#   · 챔버: bel +84 정점(f8 되누름), baz -20(오른뒤 감김. 현 -4 는 감김이 얕다),
#     bend +35, tp -12, wf -16, lg -19. [R10] Δbel 8,10,9,7,4 느린-빠른-눌러앉음.
#   · 한 박자 f7(질량 반전점) 유지.
#   · 낙하 f9~13: Δbel 12,24,38,36,13 = 중력 가속 + 오버슛 감쇠([R11]).
#     hot 은 f10~12(Δ24 부터 문턱 16도/장 초과). baz 는 f10 까지 -10(화면 세로),
#     칼이 깊어지는 f11~13 에서만 +2->+24 로 탈출([G3] 접힘 회피 + 대각 40~45도).
#     골반 선행: hy 가 f8 에서 먼저 풀리고(ty -6 일 때 hy -4) 임팩트에서 hy>ty.
#     체중: cr 14->28 주저앉음, wf +21, lg +31.
#   · 잔심 f14~16: 칼 정지(Δ<=2), 몸만 가라앉았다 편다.
#   · 회수 f17~21: Δbel 5,8,10,9,7 — cr 28->5 가 먼저 줄며 다리가 칼을 들어올린다.
#     리본 문턱(14.9도/장) 아래 유지.
#   검증: 아래 화면 자(칼길이 45% 미만 0장·죽은 장·등속), probe_blade25 날 정렬,
#   렌더 8방향 눈검증, 라이브 hot 창 실측(main.js ATK_COMMIT.Heavy 갱신).
HAND_SPEC_V26_HEAVY = (22, [
    # 챔버 f1~6: [R10] 가속 서사. baz 를 -20 까지 감아 오른뒤 장전
    # (1차 굽기 f1 bel+44 = hot 18.9 / 2차 f1 baz-6·bend+8 도 16.3 — f1 의 유령은
    #  본 스윙과 0.232초 차라 SWING_GAP(0.22) 밖 = **별도 스윙 번호**가 된다.
    #  f1 은 24차 검증치(v 9.6)와 같은 소프트 스타트로 두고 가속은 f2 부터 싣는다)
    # (3차 굽기: 감김을 f2 부터 실으니 유령이 f2 로 옮겨 갔다(17.4) — 방위 감김은
    #  **칼이 높이 선 f4~6 에서만** 싣는다. 고도 70+ 에서는 cos(bel) 이 작아 같은
    #  Δbaz 라도 칼끝이 거의 안 움직인다 = 공짜 감김.)
    (1,  dict(bel=+40, baz=-2,  bend=+7,  ty=-1, tp=-2,  hy=-2, cr=3,  lg=-4,  wf=-4,  ws=-1)),
    (2,  dict(bel=+46, baz=-4,  bend=+15, ty=-3, tp=-5,  hy=-3, cr=7,  lg=-9,  wf=-8,  ws=-2)),
    # (4차 굽기: f4 가 15.9 로 문턱(15.8)을 0.1 넘었다 — 리본 한 장 튐. 1~2도 눌러 잡는다)
    (3,  dict(bel=+54, baz=-8,  bend=+22, ty=-4, tp=-7,  hy=-4, cr=10, lg=-13, wf=-11, ws=-3)),
    (4,  dict(bel=+65, baz=-12, bend=+29, ty=-5, tp=-9,  hy=-5, cr=12, lg=-16, wf=-13, ws=-4)),
    (5,  dict(bel=+75, baz=-17, bend=+33, ty=-6, tp=-11, hy=-6, cr=13, lg=-18, wf=-15, ws=-4)),
    (6,  dict(bel=+82, baz=-20, bend=+35, ty=-7, tp=-12, hy=-6, cr=14, lg=-19, wf=-16, ws=-5)),
    # 한 박자 정지 f7 + 되누름 f8(정점 +84. [R12] 골반이 먼저 출발: hy -4, ty 는 -6)
    (7,  dict(bel=+82, baz=-20, bend=+35, ty=-7, tp=-12, hy=-6, cr=14, lg=-19, wf=-16, ws=-5)),
    (8,  dict(bel=+84, baz=-19, bend=+34, ty=-6, tp=-11, hy=-4, cr=15, lg=-16, wf=-12, ws=-4)),
    # 낙하 f9~12: 중력 가속(Δ12->24->38->36). hot 은 f10 부터. baz 는 f10 까지 참고
    # 칼이 깊어지는 곳부터 [G3] 접힘 금지구역 탈출 방위를 크게 준다(1차 굽기:
    # f12 baz+14 는 40px(37%)로 접혔다 — V24 실증치 +38 대역으로 되돌린다)
    (9,  dict(bel=+72, baz=-16, arl=+30, bend=+28, ty=-4, tp=-6, hy=-1, cr=17, lg=-8,  wf=-4,  ws=-2)),
    (10, dict(bel=+48, baz=-10, arl=+38, bend=+20, ty=-1, tp=+1, hy=+3, cr=20, lg=+6,  wf=+6,  ws=0)),
    (11, dict(bel=+10, baz=+6,  arl=+46, bend=+12, ty=+3, tp=+6, hy=+6, cr=24, lg=+18, wf=+14, ws=+3)),
    (12, dict(bel=-24, baz=+26, arl=+52, bend=+9,  ty=+5, tp=+9, hy=+7, cr=27, lg=+27, wf=+19, ws=+5)),
    # 오버슛 f13([R11] 관성. 1차 굽기 bel-39 는 z-0.119 관통 — -34 가 스침(-0.01)이다)
    (13, dict(bel=-34, baz=+38, arl=+55, bend=+8,  ty=+6, tp=+10, hy=+8, cr=28, lg=+31, wf=+21, ws=+6)),
    (14, dict(bel=-32, baz=+40, arl=+54, bend=+9,  ty=+6, tp=+10, hy=+8, cr=26, lg=+29, wf=+20, ws=+6)),
    # 잔심 f15~16: 칼은 서고 몸이 무게를 받아낸다
    (15, dict(bel=-31, baz=+40, arl=+53, bend=+10, ty=+6, tp=+9,  hy=+7, cr=24, lg=+27, wf=+18, ws=+5)),
    (16, dict(bel=-31, baz=+39, arl=+52, bend=+10, ty=+6, tp=+9,  hy=+7, cr=22, lg=+25, wf=+16, ws=+5)),
    # 회수 f17~21: 다리가 든다(cr 22->5 가 견인, bel 은 따라온다). [R10] 4,7,9,8,7
    (17, dict(bel=-27, baz=+37, arl=+49, bend=+10, ty=+5, tp=+8, hy=+6, cr=19, lg=+21, wf=+13, ws=+4)),
    (18, dict(bel=-20, baz=+33, arl=+45, bend=+10, ty=+5, tp=+6, hy=+5, cr=15, lg=+17, wf=+10, ws=+3)),
    (19, dict(bel=-11, baz=+28, arl=+41, bend=+9,  ty=+4, tp=+4, hy=+3, cr=11, lg=+13, wf=+8,  ws=+3)),
    (20, dict(bel=-3,  baz=+23, arl=+36, bend=+7,  ty=+3, tp=+3, hy=+2, cr=8,  lg=+9,  wf=+5,  ws=+2)),
    (21, dict(bel=+4,  baz=+18, arl=+32, bend=+6,  ty=+2, tp=+2, hy=+1, cr=5,  lg=+6,  wf=+4,  ws=+1)),
])
# ★26차 롤백 스위치. 0 이면 24·25차 X(HAND_SPEC_V24 의 Heavy)가 그대로 이긴다.
HEAVY_V26 = os.environ.get("HEAVY_V26", "1") == "1"
if MOVES_V24 and HEAVY_V26:
    HAND_SPEC["Heavy"] = HAND_SPEC_V26_HEAVY
