...
# ── 조명 ──
# ★★13차B(컨셉 재현). 1차가 오너에게 "미감이 하나도 없다"고 기각당한 기계적 원인은
#   조명이 아니라 **색이었다.** 실측으로 갈라 보면 이렇다.
#
#       컨셉(concept_hall.png) 전체 평균 #181e25  S 0.33  V 0.15
#       1차 게임 화면            바닥 평균 warm gray  S 0.10  V 0.45
#
#   컨셉은 (가) 훨씬 어둡고 (나) 어둠에 **채도**가 있고(깊은 구석 S 0.90 짙은 남색)
#   (다) 그 남색 위에 주황 웜 풀이 얹혀 보색 리듬을 만든다. 1차는 셋 다 없었다.
#   그래서 이번 판은 색을 세 층으로 나눠 만든다.
#
#       ① 씬 조명(main.js 던전 분기)  = 차가운 달빛.  E = (0.391, 0.565, 0.864)
#       ② 정점색(COLOR_0, 이 파일)     = 어둠의 깊이와 웜/콜드 기울기
#       ③ 이미시브 데칼(이 파일)       = 웜 풀·달빛 웅덩이의 **밝기 폭**
#
#   ③ 이 새로 생긴 층이다. 정점색만으로는 웜 풀을 못 만든다 — COLOR_0 이 1 에서
#   잘리므로 밝은 쪽이 어두운 쪽의 (1/AMB)배를 못 넘는데, 컨셉의 횃불 코어와
#   구석 어둠은 화면 선형으로 **30배** 차이다. 이미시브는 조명·정점색 계약 밖이라
#   그 폭을 자유롭게 낸다.
#
# AMB : 아무 빛도 없는 자리. 완전한 검정을 안 쓰는 이유는 캐릭터가 배경에서
#       떨어져 보이려면 바닥이 형체는 있어야 하기 때문이다.
#       ★1차보다 **더 어둡고 더 파랗다**(0.255,0.290,0.365 -> 0.20,0.25,0.36).
#         파랑/빨강 비 1.43 -> 1.80. 어둠이 회색이 아니라 남색이 되는 자리다.
#
# ★★13차D. 오너 "너무 새벽 느낌이야." 실측이 범인을 정확히 짚었다.
#   화면을 휘도 백분위로 나눠 R/B 를 재면(scripts/d2_grade.py) 컨셉과 이렇게 갈린다.
#
#                    [0~25%]  [55~80%]  [80~95%]   ← 휘도 백분위 띠
#       컨셉           0.23      0.94      1.09
#       옛 판          0.26      0.30      0.30     ← **위로 가도 파랗다**
#
#   컨셉은 "어두우면 파랗고 밝으면 따뜻하다"인데 옛 판은 **밝아져도 파랗다**.
#   눈은 그 그림을 '밤'이 아니라 '새벽 하늘빛'으로 읽는다. 파랑이 죄가 아니라
#   **밝은 자리까지 파란 것**이 죄였다.
#
#   기계적 원인은 곱셈 사슬이다.  화면 = E x 알베도 x 정점색
#       E(던전 조명) R/B 0.45   알베도(옛 팔레트) R/B 1.15   ->  둘이 곱해 0.52
#   즉 정점색이 R/B 2.1 을 넘겨야 화면이 겨우 중립이 되는데, 옛 AMB 는 파랑이
#   0.305 나 되어 분모로 눌러 앉아 있었다(횃불 f 가 0.82 는 돼야 넘는다 = 불꽃 코앞뿐).
#
#   그래서 셋을 같이 돌린다.
#     (가) AMB 를 **깊게** 내리고 파랑 비를 더 준다(어둠은 더 어둡고 더 남색)
#     (나) 팔레트를 **따뜻한 쪽으로** 돌린다(알베도 R/B 1.15 -> 1.7 안팎)
#     (다) 횃불 색을 더 진한 주황으로(컨셉 횃불 둘레 실측 R/B 8.8 · 코어 18.6)
#   (가)+(나) 가 짝이다. (나)만 하면 어둠까지 갈색이 되고, (가)만 하면 밝은 자리가
#   여전히 파랗다.
#   ★1차 시도 실측이 이 값을 한 번 더 밀었다: 팔레트를 따뜻하게 돌리자 **어둠까지**
#     따뜻해졌다(휘도 하위 25% 띠의 화면 R/B 0.25 -> 0.43. 컨셉은 0.23 이다).
#     알베도가 R/B 1.7 이므로 어둠을 컨셉만큼 파랗게 두려면 정점색이 그만큼 더
#     파래야 한다. 파랑은 그대로 두고 빨강·초록만 내린다(어둠이 더 어두워지는 덤).
AMB = np.array((0.034, 0.076, 0.196), np.float32)
# ★AMB_MIN 도 같이 내린다. 0.135 는 높이 감쇠(x0.42)를 먹은 벽 꼭대기를 도로
#   0.135 로 들어 올려서 **벽 상단이 어둠에 안 녹았다**(그 자리가 R 도 B 도 0.135 라
#   회색으로 떴다). 컨셉의 벽 꼭대기 휘도는 0.005 다.
AMB_MIN = 0.028       # 어떤 채널도 이보다 어두워지지 않는다
# ★13차D. (1.00,0.56,0.20) -> 더 진한 주황. 컨셉 횃불 둘레 벽의 R/B 가 8.8 이다.
TORCH_RGB = np.array((1.00, 0.50, 0.14), np.float32)   # 횃불 빛의 색
COLD_RGB = np.array((0.34, 0.60, 1.00), np.float32)    # 계단·천장 틈의 찬 빛
MOSS_RGB = np.array((0.10, 0.26, 0.06), np.float32)    # 벽 밑동 이끼(정점색에 더한다)
# ★13차C. 4.3 -> 3.6. 옛 값은 꼬리가 5m 넘게 완만해서(실측 벽 R/B 0.6m 1.66 ->
#   3.6m 1.19) 횃불이 '안개'였다. 컨셉의 횃불은 1m 안이 황백이고 2m 밖은 이미
#   남색이다 - '가까이 세다'가 아니라 '**빨리 죽는다**'가 불의 문법이고, 그 대비가
#   있어야 빛이 정보가 된다. 평균 밝기는 재질 곱수가 자동으로 되받는다(곱수 계약).
# ★13차D. 3.6 -> 3.8. 어둠을 깊게 내렸으니(AMB) 같은 반경이면 웜 존이 상대적으로
#   좁아 보인다. 컨셉의 홀 한복판은 횃불에서 3~4m 떨어졌는데도 올리브색이다
#   (실측 R/B 0.67) - 그 넓고 옅은 웜이 '동굴 안 공기'다. 대신 세기는 아래 창(w^2.7)이
#   그대로라 꼬리는 여전히 빨리 죽는다.
TORCH_R = 3.8         # 횃불 반경(m). 이 거리에서 세기의 1/2 x 창 이 남는다
TORCH_P = 1.06        # 횃불 세기
# ★★꼬리를 자르는 창(window). 1/(1+(d/R)^2) 만 쓰면 꼬리가 길어서 **광원 마흔 개가
#   서로를 더해 맵 전체가 1.0 으로 포화**된다(첫 판에서 실제로 그랬다:
#   바닥 정점밝기 최소 0.96 · 최대 1.00 = 리듬 0). 사거리 밖은 0 이 되게 잘라야
#   "웅덩이"가 생긴다.
# ★13차D. 2.3 -> 2.5. 판정지에서 두 가지가 같이 잡혔다.
#   (가) 휘도 중간 띠의 R/B 가 컨셉 0.94 인데 우리는 0.70 이다 - 컨셉의 홀 한복판은
#       횃불에서 3~4m 떨어져도 올리브색이고(실측 0.67), 그 넓고 옅은 웜이
#       '동굴 안 공기'다. 우리는 웅덩이 밖이 곧장 남색이었다.
#   (나) 통로 한복판이 걸어 다니기에 너무 어두웠다(같은 원인의 뒷면).
#   ★13차C 가 이 값을 조인 이유("꼬리가 길면 안개")는 **옛 AMB 에서만** 맞았다.
#     그때는 바닥이 이미 밝은 파랑이라 옅은 꼬리가 그 위에 안개로 얹혔다. 지금은
#     바닥이 깊은 남색이라 같은 꼬리가 '멀리서 오는 불빛'으로 읽힌다.
#     창 지수(w^2.7)는 그대로다 = 가까운 쪽 대비는 안 잃는다.
TORCH_RANGE = 2.5     # 사거리 = R 의 몇 배까지 빛이 닿는가
TOP_BONUS = 0.10      # 벽 마루(윗면)에 얹는 밝기. 두께가 읽히게 한다

# ★★높이 감쇠 — 쿼터뷰에서 **벽 상단이 어둠으로 사라지게** 하는 장치.
#   컨셉에서 벽은 위로 갈수록 검어지다가 천장 없는 어둠에 녹는다. 1차는 벽이
#   위아래 같은 밝기라 "회색 상자"의 윗면이 그대로 드러났다(하늘 샘의 인상도 여기서 왔다).
#   횃불이 2.05m 에 걸리므로 그보다 위는 실제로도 빛이 없다 - 물리적으로도 맞다.
HFALL_Y0 = 2.20       # 여기까지는 안 건드린다
HFALL_SPAN = 3.20     # 이만큼 올라가는 동안
HFALL_MIN = 0.42      # 여기까지 어두워진다(테두리 벽 7.5m 꼭대기가 이 값)
...
NEAR_NL = 0.05        # 등진 면에 남기는 몫. 0.05 = 사실상 안 남긴다(뒷면은 차다)
...
MACRO_A = 0.175       # 큰 얼룩 진폭(곱수). 밝은 마모 존 / 어두운 습윤 존
...
CONTACT_AO = 0.30     # 벽에 딱 붙은 자리에서 몇 % 어두워지는가
...
WALL_UV_SCALE = 2.8        # 벽. 블록 한 단이 0.40m 가 되는 값
...
# 6) 팔레트 · 재질
# ═════════════════════════════════════════════════════════════
# ★★13차B. 팔레트를 **감으로 안 적었다.** 컨셉 그림의 구역별 화면색을 재고,
#   던전 조명(main.js 던전 분기)의 조도로 ACES 를 역산해 알베도로 되돌린 값이다.
#
#     화면색 = sRGB( ACES( E x 알베도 ) )        E = (0.391, 0.565, 0.864)
#     알베도 = screen_to_paint(화면목표, E)       tools/color_contract.py
#
#   E 는 던전 조명 세 개(반구 0x6f9ad2 1.70 · 키 0xc4d8f0 1.70 · 림 0x3f6ea6 0.45)의
#   윗면 조도를 초원 실측(IRRADIANCE 0.99)과 같은 자로 환산한 값이다. 휘도 0.55 =
#   초원의 55%. **차고(파랑/빨강 2.2배) 어둡다.**
#
#   컨셉 실측(concept_hall.png) -> 이 맵의 화면 목표
#     전체 평균 #181e25 (S .33 V .15)   어둠 바닥 #1a1f28   웜풀 바닥 #383f38
#     달빛 바닥 #26578e (S .73 V .56)   벽 어둠   #131529
#
#   ★1차와의 차이가 여기 다 있다: 1차 바닥 목표는 #302d29 = R>G>B **따뜻한 회갈색**,
#     이번은 R<G<B 로 뒤집힌 **찬 돌**이다. 그리고 화면 V 를 45% 에서 20% 로 내렸다.
#
# ★★13차D 개정. 오너 "너무 새벽 느낌이야."
#   위 AMB 주석의 곱셈 사슬을 보라: 화면 = E x 알베도 x 정점색 이고 E 의 R/B 가
#   0.45 다. 알베도까지 R/B 1.15 로 차게 칠하면 **정점색이 R/B 2.1 을 넘기기 전엔
#   화면이 파랗다** - 그 문턱은 불꽃 코앞에서만 넘는다. 그래서 알베도를 따뜻한 쪽으로
#   돌린다(R/B 1.15 -> 1.6~1.9 · G/B 1.28~1.40). 이 값은
#   `python3 tools/color_contract.py --dungeon fwd <hex>` 로 화면색을 확인하고 뽑았다.
#
#   ★"화면 평균이 회색이 되면 컨셉(#181e25, 파랑 S 35%)과 어긋나지 않나"
#     — 안 어긋난다. 곱수 계약이 묶는 것은 **면적가중 평균 하나**뿐이다. 어둠이
#     화면의 3분의 2 를 파랑으로 덮고 나머지가 횃불 주황이면, 평균이 회색이어도
#     눈은 파랑과 주황 둘로 본다. 컨셉이 정확히 그 구조다(어둠 R/B 0.02~0.12 /
#     횃불 둘레 8.8). 평균색을 파랗게 칠하면 그건 **어둠이 아니라 전체가** 파래진다.
#
#   ★밝기도 같이 내렸다(휘도 x0.74~0.90). AMB 를 깊게 내렸으므로 정점색 평균이
#     떨어지고, 계약상 곱수가 그만큼 올라가는데 옛 목표 그대로면 MAT_DG_CUT 이
#     1.0 에서 잘린다(= 계약 파기). 목표를 내려서 곱수에 자리를 만든다.
PAL = {
    # (알베도hex, 화면목표hex) - 화면목표는 근거를 남기려고 같이 적는다
    "floor":  "473f31",     # 화면 #1c1e20  판석 바닥
    "dirt":   "3d362a",     # 화면 #141717  잔해 둘레 부스러기
    "rubble": "4a4133",     # 화면 #1e2022  부서진 석재(바닥보다 밝다 = 얹힌 물건)
    "wall":   "363124",     # 화면 #111314  벽 석재. 바닥보다 어둡다
    "cut":    "4d4436",     # 화면 #212427  기둥·아치·문설주. 사람이 다듬은 돌
    "altar":  "594d3c",     # 화면 #2c2d2e  제단. 가장 따뜻한 돌
    "stair":  "424442",     # 화면 #182330  계단. 유일하게 안 따뜻한 돌(출구는 차다)
    "iron":   "332c26",     # 화면 #0c0e11  검은 쇠
    "banner": "503b2f",     # 화면 #211b1b  벽걸이 깃발(낡은 진홍)
    "flame":  "ffb648",     # 이미시브. 조명도 정점색도 안 탄다
}

# ★타일의 **색기를 얼마나 살릴 것인가**(0 = 목표색으로 완전히 끌어당김 / 1 = 타일 그대로).
#   컨셉 타일의 청록 돌 + 이끼 초록이 이 판의 미감이라 절반 넘게 살린다.
#   1차는 이 손잡이가 없어서(항상 0) 절차 타일의 회색이 그대로 목표색이 됐다.
HUE_KEEP = 0.55
...
for _ti, (gx, gz, gy, dx, dz, ped) in enumerate(TORCH_PROPS):
    ...
    if ped:
        ...
        add_pool(gx + dx * 0.34, gz + dz * 0.34, 2.35)
        add_halo(gx + dx * 0.24, gz + dz * 0.24, gy + 0.06)
        add_wall_glow(gx, gz, dx, dz, gy, back=0.0)

# ── 제단 화로 · 모닥불의 웜 풀 ──
add_pool(ALTAR_X - 2.4, ALTAR_Z + 0.2, 3.10)
add_pool(ALTAR_X + 2.4, ALTAR_Z + 0.2, 3.10)
add_pool(CAMP_X, CAMP_Z, 2.90)
...
# ★13차C. 바닥만 hue_keep 을 0.55 -> 0.40 으로 내린다. 타일 채도를 44% 뺐더니
#   초록이 상쇄하던 분홍 돌기가 드러나 바닥이 **라벤더**로 떴다(증거 SHEET_floor_ab
#   접사). hue_keep 을 내리면 채널별 보정이 세져서 평균이 목표 회색(#494745)에 더
#   붙는다 = 돌은 무채색이 되고 색은 횃불·달빛이 칠한다(이 판의 문법 그대로다).
# ★13차D. 바닥 0.40 -> 0.30 · 벽 0.34 -> 0.22 · 기둥 0.55 -> 0.38.
#   hue_keep 은 **타일의 색기를 얼마나 살리는가**인데, 살리는 그 색기가 하필
#   찬 청록이다(dg_floor 타일 R/B 0.94 · dg_wall 0.76). 팔레트를 따뜻하게 돌려도
#   hue_keep 이 그걸 도로 절반 끌어내린다(1차 시도에서 목표 R/B 1.72 가 화면
#   1.49 로, 벽은 1.60 목표가 1.07 로 주저앉았다). 돌의 결·얼룩은 명암이 들고
#   있으므로 색기는 덜 살려도 손해가 작다.
buf_floor.mat = mat_tex("MAT_DG_FLOOR", PAL["floor"], IMG_FLOOR, FLOOR_LIN,
                        shade=buf_floor.shade, hue_keep=0.30)
buf_floorb.mat = mat_tex("MAT_DG_FLOORB", PAL["floor"], IMG_FLOOR_B, FLOOR_B_LIN,
                         shade=buf_floorb.shade, hue_keep=0.30)
buf_dirt.mat = mat_tex("MAT_DG_DIRT", PAL["dirt"], IMG_FLOOR_B, FLOOR_B_LIN,
                       shade=buf_dirt.shade)
buf_wall.mat = mat_tex("MAT_DG_WALL", PAL["wall"], IMG_WALL, WALL_LIN,
                       shade=buf_wall.shade, hue_keep=0.22)
buf_cut.mat = mat_tex("MAT_DG_CUT", PAL["cut"], IMG_WALL, WALL_LIN,
                      shade=buf_cut.shade, hue_keep=0.38)
buf_altar.mat = mat_tex("MAT_DG_ALTAR", PAL["altar"], IMG_WALL, WALL_LIN,
                        shade=buf_altar.shade)
buf_stair.mat = mat_tex("MAT_DG_STAIR", PAL["stair"], IMG_WALL, WALL_LIN,
                        shade=buf_stair.shade)
buf_rubble.mat = mat_tex("MAT_DG_RUBBLE", PAL["rubble"], IMG_WALL, WALL_LIN,
                         shade=buf_rubble.shade)
...
buf_medal.mat = mat_decal("MAT_DG_MEDAL", PAL["cut"], IMG_MEDAL, MEDAL_LIN,
                          shade=(1.0, 1.0, 1.0), emit=0.45)
# ── 빛 자체 ──
...
#   0.74 x 알파 0.62 = 0.46 선형 < 블룸 임계 1.02 (안 번진다)
buf_pool.mat = mat_glow("MAT_DG_POOL", IMG_POOL, 0.74)
# ★13차D. 0.62 -> 0.50. 콘을 줄이고 웅덩이만 두면 **바닥에 놓인 파란 전구**가 된다
#   (1차 시도 캡처가 그랬다). 빛줄기와 웅덩이는 세기 비가 유지돼야 위에서 내려온
#   빛으로 읽힌다 - 둘을 같이 내리고 콘 쪽을 덜 내린다.
buf_poolc.mat = mat_glow("MAT_DG_POOLC", IMG_POOLC, 0.50)
# ★13차D. 0.52 -> 0.34. 오너 "달빛 샤프트는 국소 연출로만." 콘은 6m 짜리 반투명
#   덩어리라 세기가 화면 넓이로 직결된다 - 바닥 웅덩이(POOLC)는 살리고 **공중
#   덩어리만** 줄여야 "빛줄기"가 남고 "파란 안개"가 걷힌다.
buf_shaft.mat = mat_glow("MAT_DG_SHAFT", IMG_SHAFT, 0.42)
buf_dust.mat = mat_glow("MAT_DG_DUST", IMG_POOLC, 1.15)
...
buf_halo.mat = mat_glow("MAT_DG_HALO", IMG_POOL, 0.96)
