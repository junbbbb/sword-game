...
1. **판석 배합을 표로 못 박는다.** 기준 모듈 **0.25m**(우리 칸 2.0m 의 1/8), 크기는 그 정수배만.
   ...
   | 크기 | 비중 | 화면 크기 (64 px/m) | 텍스처 크기 (1024/5.0m) |
   |---|---|---|---|
   | 0.25 m | **15%** | 16 px | 51 px |
   | 0.50 m | **50%** | 32 px | 102 px |
   | 0.75 m | **35%** | 48 px | 154 px |

   → 5.0m 타일 한 장에 **약 75장**(지금 19~32장의 2.5~4배).
   폴리카운트 권고가 하한 16, HOTS 급 10²=100 이니 그 사이다.
   4종으로 갈 거면 0.25/0.50/0.75/1.00 에 **15 / 40 / 35 / 10**, 이때는 약 62장이다.
   ★**균등 분포로 뽑지 말 것. 가운데 크기가 절반을 먹어야 한다.**
   ★**큰 돌을 먼저 다 놓고 남는 틈을 작은 돌로 메운다**(반대로 하면 큰 돌이 안 들어간다).
   ★가장자리 낱개 조각만 0.15m 까지 내려가도 된다(그때 최대/최소비가 롤 실측 5~6배에 닿는다).
2. **격자는 유지하되 칸마다 사각형 한 장을 찍지 않는다.** 크기가 전부 한 모듈의 정수배라
   격자 정합은 깨지지 않는다. 상업 포장재가 정확히 이 구조다(300mm 격자에 150mm 배수).
3. **모양은 불규칙 다각형(4~7각), 가로세로비 2.6:1 이하.** 둥근 아메바를 버린다.
   ★14차 함정("절차 블록은 둥글기·곱셈 하이라이트가 알약·젤리곰을 만든다")이 바닥에도 그대로 있다.
4. **줄눈 두 규칙을 코드로 강제한다.**
   * 네 모서리가 한 점에서 만나면 안 된다
   * 줄눈이 **3.0m** 넘게 직선으로 이어지면 안 된다 (= 우리 칸 1.5칸)
5. **줄눈을 얇고 조용하게.** 폭은 판 폭의 **1.7~2%**(600mm 돌에 10~12mm 라는 실물 규격),
...
6. **줄눈은 판 윗면보다 함몰시킨다.** 실물 규격 1/4인치 ≈ 판 폭의 **1.5~2%**.
   기하로 낮출 수 없으면 텍스처의 값과 버텍스 AO 로 흉내 낸다.
   ★위에서 내리쬐는 우리 카메라에서 **판을 서로 떼어 놓는 것이 바로 이 함몰**이다.
7. **판마다 위가 밝고 아래가 어둡다.** 큰 기울기 한 장(타일 전체) + **판 하나 안의 작은 기울기**,
...
8. **에지 하이라이트는 일부에만, 굵기를 흩어서.** 폴리카운트 정설:
...
9. **파손은 등급으로.** 대부분은 **모서리만 살짝 깨진 것**, 소수만 **완전히 부서진 것**,
    나머지는 멀쩡한 것. 그리고 **두께도 흩는다**(전부 같은 두께면 어색해진다).
10. **판마다 밑값을 흩는다.** 판석끼리 밝기 ±6%, 색상 ±4°.
    ★이게 "판석대 σ" 를 잡음이 아니라 **정보**로 채우는 방법이다.
11. **매크로 얼룩을 키운다.** 손잡이는 이미 있다 — `s40_dungeon1.py` 의
...
12. **닳은 길.** 통로 중앙과 방 사이 동선에 저주파 마스크를 깔아 **줄눈 이끼를 지우고 높이차를 눌러**
    평평하게 만든다. 안 닳은 구석과의 대비가 그림이 된다.
13. **최소 형상 23px 규칙.** 1024/5.0m 기준, **텍스처에 23px 미만 형상을 그리지 않는다**
    (화면 64px/m 에서 0.11m 미만은 지글거림). 그 아래는 전부 저주파 얼룩으로 대체.
14. **이음매 감사법.** 포토샵 offset 을 텍스처 크기의 **50%**(1024면 512)로 밀어 가장자리를
